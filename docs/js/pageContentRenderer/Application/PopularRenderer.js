...
// モジュール記号  : POPRENDR / PopRendr
// モジュール名    : 人気記事セクション生成
// 内容            : viewStatsから人気記事カードを描画する
// Copyright(c) 2025 Fibrantix CO.,LTD. All Rights Reserved
// ----------------------------------------------------------------------------

import { HTMLHLPR_EscapeHtml, HTMLHLPR_EnsureArray } from '../Middleware/HtmlHelper.js';
 ...
 */
export function POPRENDR_RenderPopularSection(allData, viewStats) {
    // allData を配列に統一
    allData = HTMLHLPR_EnsureArray(allData);
    // viewStats を配列に統一
    viewStats = HTMLHLPR_EnsureArray(viewStats);
    ...
    viewStats.forEach(item => {
        ...
        const viewStatsThumb = PREVLINK_BuildThumbHtml(node, 'popular-entry-card-thumb widget-entry-card-thumb');

        html += `<a href="${item.url}" class="popular-entry-card-link widget-entry-card-link a-wrap no-1" title="${HTMLHLPR_EscapeHtml(item.label||'')}" data-nodal="">`;
        ...
        html += `<div class="popular-entry-card-title widget-entry-card-title card-title">${HTMLHLPR_EscapeHtml(item.label||node.label||'')}</div>`;
        html += `<div class="popular-entry-card-date widget-entry-card-date display-none">`;
        html += `<span class="popular-entry-card-post-date widget-entry-card-post-date post-date">${HTMLHLPR_EscapeHtml(node.datePublished||'')}</span>`;
        html += `<span class="popular-entry-card-update-date widget-entry-card-update-date post-update">${HTMLHLPR_EscapeHtml(node.dateModified||'')}</span>`;
        html += `</div>`;
    ...
    html += `</div>`;
    DOMWRITER_WriteToContainer('popular_entries', html);
}
